...
#jnas_data=jnas_data
#jnas_exp=jnas_exp
nnet3_dir=$exp/nnet3${nnet3_affix}
...
if [ $stage -le 2 ]; then
  ...
  for data_set in ${train_set}_sp ${test_sets}; do
  ...
  for data_set in ${train_set}_sp ${test_sets}; do
    steps/make_mfcc.sh --nj $nj --mfcc-config conf/mfcc_hires.conf \
      --cmd "$train_cmd" $data/${data_set}_hires
...
if [ $stage -le 3 ]; then
  ...
  mkdir -p $nnet3_dir/diag_ubm
  temp_data_root=$nnet3_dir/diag_ubm
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
    ...
    $nnet3_dir/pca_transform $nnet3_dir/diag_ubm
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
    $data/${train_set}_sp_hires $nnet3_dir/diag_ubm $nnet3_dir/extractor || exit 1;
fi
...
if [ $stage -le 5 ]; then
  ...
  ivectordir=$nnet3_dir/ivectors_${train_set}_sp_hires
  ...
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
    ...
    $nnet3_dir/extractor $ivectordir
  ...
  for test_set in ${test_sets}; do
    ...
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj "${nspk}" \
      $data/${est_set}_hires $nnet3_dir/extractor \
      $nnet3_dir/ivectors_${test_set}_hires
  done
fi
...
if [ $stage -le 8 ]; then
  ...
  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
    $data/${train_set}_sp $data/lang_combined $gmm_dir $ali_dir
fi
