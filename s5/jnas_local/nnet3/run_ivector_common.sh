...
for f in $jnas_data/${train_set}/feats.scp ${gmm_dir}/final.mdl; do
  if [ ! -f $f ]; then
...
if [ $stage -le 3 ]; then
  ...
  mfccdir=$jnas_data/${train_set}_sp_hires/data
  ...
  for datadir in ${train_set}_sp $dev_set ${test_sets}; do
    utils/copy_data_dir.sh $jnas_data/$datadir $jnas_data/${datadir}_hires
  done
...
if [ $stage -le 4 ]; then
  ...
  mkdir -p $jnas_exp/nnet3${nnet3_affix}/diag_ubm
  temp_data_root=$jnas_exp/nnet3${nnet3_affix}/diag_ubm

  num_utts_total=$(wc -l <$jnas_data/${train_set}_sp_hires/utt2spk)
  num_utts=$[$num_utts_total/3]
  utils/data/subset_data_dir.sh $jnas_data/${train_set}_sp_hires \
  ...
  steps/online/nnet2/get_pca_transform.sh --cmd "$train_cmd" \
    ...
    $jnas_exp/nnet3${nnet3_affix}/pca_transform
