...
$jnas_local/nnet3/run_ivector_common.sh --stage $stage \
                                  --train-set $train_set \
                                  --gmm $gmm \
                                  --nnet3-affix "$nnet3_affix" || exit 1;

gmm_dir=$jnas_exp/$gmm
ali_dir=$jnas_exp/${gmm}_ali_${train_set}_sp
tree_dir=$jnas_exp/chain${nnet3_affix}/tree${tree_affix:+_$tree_affix}
lang=$csj_data/lang_chain
lat_dir=$jnas_exp/chain${nnet3_affix}/${gmm}_${train_set}_sp_lats
dir=$jnas_exp/chain${nnet3_affix}/tdnn${affix}
train_data_dir=$jnas_data/${train_set}_sp_hires
lores_train_data_dir=$jnas_data/${train_set}_sp
train_ivector_dir=$jnas_exp/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires
...
if [ $stage -le 14 ]; then
  utils/mkgraph.sh \
    --self-loop-scale 1.0 $csj_data/lang_csj_tg $dir $dir/graph_csj_tg

  for decode_set in $test_sets; do
    steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 --nj 10 \
      --cmd "$decode_cmd" \
      --online-ivector-dir $jnas_exp/nnet3${nnet3_affix}/ivectors_${decode_set}_hires \
      $dir/graph_csj_tg $jnas_data/${decode_set}_hires $dir/decode_${decode_set}
  done

  steps/online/nnet3/prepare_online_decoding.sh \
    --mfcc-config conf/mfcc_hires.conf $lang $jnas_exp/nnet3${nnet3_affix}/extractor \
    $dir ${dir}_online

  for decode_set in $test_sets; do
    steps/online/nnet3/decode.sh --nj 10 --cmd "$decode_cmd" \
      --acwt 1.0 --post-decode-acwt 10.0 \
      $dir/graph_csj_tg $jnas_data/${decode_set}_hires ${dir}_online/decode_${decode_set}
  done
fi
exit 0;
