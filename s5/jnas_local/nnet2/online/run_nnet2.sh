  ...
  for lm_suffix in csj_tg; do
    ...
    for data in eval1 eval2 eval3; do
      steps/nnet2/decode.sh --nj 8 --cmd "$decode_cmd" \
         ...
         $graph_dir data/${data}_hires $dir/decode_${lm_suffix}_${data} || exit 1;
    done
  done
fi
  ...
  for lm_suffix in csj_tg; do
    ...
    for data in eval1 eval2 eval3; do
      steps/online/nnet2/decode.sh --cmd "$decode_cmd" --nj 8 \
        "$graph_dir" data/${data} ${dir}_online/decode_${lm_suffix}_${data} || exit 1;
    done
  done
fi
...
if [ $stage -le 12 ]; then
  ...
  for lm_suffix in csj_tg; do
    ...
    for data in eval1 eval2 eval3; do
      steps/online/nnet2/decode.sh --cmd "$decode_cmd" --nj 8 \
        --per-utt true \
        "$graph_dir" data/${data} ${dir}_online/decode_${lm_suffix}_${data}_utt || exit 1;
    done
  done
fi
  ...
  for lm_suffix in csj_tg; do
    ...
    for data in eval1 eval2 eval3; do
      steps/online/nnet2/decode.sh --cmd "$decode_cmd" --nj 8 \
        ...
        "$graph_dir" data/${data} ${dir}_online/decode_${lm_suffix}_${data}_utt_offline || exit 1;
    done
  done
fi
...
if [ $stage -le 14 ]; then
  ...
  for year in eval1 eval2 eval3; do
    steps/online/nnet2/extract_ivectors.sh --cmd "$train_cmd" --nj 8 \
      --sub-speaker-frames 1500 \
      data/${data}_hires data/lang exp/nnet2_online/extractor \
