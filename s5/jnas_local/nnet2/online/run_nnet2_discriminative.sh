...
if [ $stage -le 3 ]; then
  steps/nnet2/train_discriminative.sh --cmd "run.pl" --learning-rate 0.0002 \
    --stage $train_stage \
    --online-ivector-dir exp/nnet2_online/ivectors_train \
    --num-jobs-nnet 4  --num-threads $num_threads --parallel-opts "$gpu_opts" \
    data/train_hires_nodup data/lang \
...
if [ $stage -le 4 ]; then
  ...
  for epoch in 1 2 3 4; do
    ...
    for lm_suffix in csj_tg; do
      graph_dir=exp/tri4/graph_${lm_suffix}
      for data in eval1 eval2 eval3; do
        steps/online/nnet2/decode.sh --cmd run.pl --nj 8 --iter smbr_epoch${epoch} \
          "$graph_dir" data/${data} ${srcdir}_online/decode_${lm_suffix}_${data}_smbr_epoch${epoch} || touch $error_file &
      done
    done
  done
  wait
